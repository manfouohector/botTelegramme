...
let isFootballRunning = false;
let isTechNewsRunning = false;
...
async function runFootballPipeline(dateStr) {
  if (isFootballRunning) {
    console.log('[PIPELINE] Pipeline Football déjà en cours d\'exécution. Abandon.');
    return { error: 'Pipeline déjà en cours' };
  }
  isFootballRunning = true;
...
async function runTechNewsPipeline(dateStr) {
  if (isTechNewsRunning) {
    console.log('[PIPELINE] Pipeline Tech News déjà en cours d\'exécution. Abandon.');
    return { error: 'Pipeline déjà en cours' };
  }
  isTechNewsRunning = true;
